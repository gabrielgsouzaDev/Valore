            ...
            <div className="space-y-1.5 mt-2">
                {d.breakdown && Object.entries(d.breakdown).map(([card, val]) => (
                    <div key={card} className="flex items-center justify-between gap-4">
                        ...
                        <span className="text-[10px] font-black text-foreground">{formatCurrency(val)}</span>
                    </div>
                ))}
            </div>
...
function MonthDetailPanel({ month, totalProjected, maxMonth, formatCurrency }: MonthDetailPanelProps) {
 ...
 */
export function InvoiceProjection({ data, selectedMonthIndex, onMonthSelect, formatCurrency }: InvoiceProjectionProps) {
            ...
            <Card className="bg-card border-border overflow-hidden">
                <CardHeader className="p-4 sm:p-6 bg-muted/30 border-b border-border/50">
                    <div className="flex items-center justify-between">
                        <div>
                            <CardTitle className="flex items-center gap-2 text-foreground text-base font-black tracking-tight uppercase italic">
                                        ...
                                        <Bar dataKey="total" radius={[8, 8, 0, 0]} barSize={40}>
                                            {data.map((_, index) => (
                                                <Cell
                                                    ...
                                                    stroke={index === selectedMonthIndex ? "var(--primary)" : "transparent"}
